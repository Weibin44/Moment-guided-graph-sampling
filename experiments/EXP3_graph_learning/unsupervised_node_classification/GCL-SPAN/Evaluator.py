...
from evaluation_protocol import get_split as shared_get_split


###################### Data Split ######################

def get_split(num_samples: int, train_ratio: float = 0.1,
              test_ratio: float = 0.8, seed: int = 15):
    if train_ratio != 0.1 or test_ratio != 0.8:
        raise ValueError('EXP3 uses the fixed 10/10/80 evaluation protocol.')
    return shared_get_split(num_samples, seed)
...
class LREvaluator(BaseEvaluator):
    def __init__(self, num_epochs: int = 500, learning_rate: float = 0.001,
    ...
    def evaluate(self, x: torch.FloatTensor, y: torch.LongTensor, split: dict):
        ...
        with tqdm(total=self.num_epochs, desc='(LR)+{:.4f}decay'.format(self.weight_decay),
            ...
            for epoch in range(self.num_epochs):
                ...
                if (epoch + 1) % self.test_interval == 0:
                    ...
                    pbar.set_postfix({'Val accuracy': best_val_acc})
                    pbar.update(self.test_interval)

        if best_state is None:
            raise RuntimeError(
                'No classifier checkpoint was evaluated. Require '
                'num_epochs >= test_interval.'
            )
        ...
        best_test_micro = f1_score(y_test, y_pred, average='micro')
        best_test_macro = f1_score(y_test, y_pred, average='macro')
