...
#                 pkl.dump(res, file)
    ...
    def augment(self, g: Graph, batch: torch.Tensor) -> Graph:
        ...
        added = modified_adj > ori_adj
        deleted = modified_adj < ori_adj
        added_pairs = torch.logical_or(added, added.transpose(-1, -2))
        deleted_pairs = torch.logical_or(deleted, deleted.transpose(-1, -2))
        asymmetric = modified_adj != modified_adj.transpose(-1, -2)
        self.last_edit_counts = {
            'add_pairs': int(torch.triu(added_pairs, diagonal=1).sum().item()),
            'delete_pairs': int(torch.triu(deleted_pairs, diagonal=1).sum().item()),
            'directed_add': int(added.sum().item()),
            'directed_delete': int(deleted.sum().item()),
            'asymmetric_pairs': int(
                torch.triu(asymmetric, diagonal=1).sum().item()
            ),
        }
